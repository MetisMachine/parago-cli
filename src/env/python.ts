...
export class Python extends Environment {
  ...
  async setup(directory:string = process.cwd()) {
    let virtualenv = shell.which('conda')

    shell.config.silent = false;

    console.log("Checking for python's conda");
    if(virtualenv == null) {
      let response = await cli.prompt('Conda is not available, install it now? [y/n]', {default: 'y'})
      
      if(['y', 'yes'].includes(response.toLowerCase())) {
        shell.exec("brew cask install miniconda")
    ...
    console.log("preparing to create conda environemt for python")

    let venv  = virtualenv.toString()
    let dir   = directory == process.cwd() ? path.join(directory, '.venv') : directory

    if(!fs.existsSync(dir)) {
      let cmd = "conda create -n " + dir + " python=3.6"
    
      console.log(`Creating conda environment at ${dir}`)
      shell.exec(cmd)
    ...
    console.log(`conda activate ${dir} && pip install -r requirements.txt\n\n`)
  }
...
//   if(virtualenv == null) {
//     let response = await cli.prompt('virtualenv is not available, install it now? [y/n]', {default: 'y'})
...
//   if(!fs.existsSync(dir)) {
//     let cmd = "virtualenv -p python3.6" + dir + " --no-site-packages"
